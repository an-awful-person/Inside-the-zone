...
export default class ITZCoreCharacterSheet extends ActorSheet {
    ...
    async getData(options) {
        ...
        this.initBackpackSpace(context['systemData']);
        this.initProtectiveDice();
        this.initSanityDice();
        this.initSkills();
        ...
        this.actor['image_class'].sanity = this.changeImageClass(this.actor['system'].sanity);
        this.actor['image_class'].health = this.changeImageClass(this.actor['system'].health);
    ...
    activateListeners(html) {
        ...
        Object.values(STATS).forEach(stat => {
            html.find(`#decrease-${stat}-button`).on("click", () => {
                ...
                this.actor['image_class'][stat] = this.changeImageClass(this.actor['system'][stat]);
            });
         ...
         * sanity roll
         */
        html.find("#actor-sanity-roll").click(this.rollSanity.bind(this));
        ...
        if (this.actor['system'].skills) {
            this.actor['system'].skills.forEach(skill => {
                html.find(`#actor-${skill.name}-increase`).click(() => this.increaseSkill(skill.name));
                html.find(`#actor-${skill.name}-decrease`).click(() => this.decreaseSkill(skill.name));
     ...
     * @private
     */
    private initSanityDice() {
        this.actor['roll'].sanity = new Attribute('1d6');
    }

    private initProtectiveDice() {
        this.actor['roll'] = {};
    ...
    private initAvatarImages(systemData) {
        /**
         * AVATAR
         */
     ...
     * @private
     */
    private increaseStat(statName:string):void {
     ...
     * @private
     */
    private decreaseStat(statName:string):void {
        const stat = this.actor['system'][statName];
     ...
     * @private
     */
    private increaseSkill(skillName:string):void {
     ...
     * @private
     */
    private decreaseSkill(skillName:string) : void {
        const filteredSkill = this.actor['system'].skills.filter(skill => skill.name == skillName)
     ...
     */
    private changeImageClass(stat):string {
    ...
    private rollSanity() {
        this.actor['roll'].sanity.rollTheDice().then(result => {
            if (result.total < 4) {
                this.decreaseStat(STATS.SANITY);
            }
        })
    }
    ...
    _onDragStart(event) {
        ...
        if (li.classList.contains('inventory')) {
            ...
            if (li.dataset.documentId) {
                const item = this.actor.items.get(li.dataset.documentId) as Item;
        ...
        if (element.classList.contains('inventory')) {
            ...
            if (backpackItems.length >= dropY + height && backpackItems[0].length >= dropX + width) {
                ...
                if (hasNoItem) {
                    ...
                    //drop the item
                    const inventoryItem = this.onDropItemCreate(itemData);
                    ...
                    if (sameItemId) {
                        const sameItem = this.actor.items.get(sameItemId) as Item;
                        sameItem.delete()
                    }
 ...
 * @private
 */
    private async onDropItemCreate(itemData) {
     ...
     * @private
     */
    private async onClickOpenItem(event) {
        let li = event.currentTarget;
        const document = this.actor.items.get(li.dataset.documentId) as Item;
        const sheet = document.sheet as FormApplication<FormApplicationOptions, FormApplication.Data<{}, FormApplicationOptions>, {}> ;
