        ...
        if (element.classList.contains('inventory')) {
            ...
            if (backpackItems.length >= dropY + height && backpackItems[0].length >= dropX + width) {
                ...
                if (hasNoItem) {
                    ...
                    if (sameItemId) {
                        const sameItem = this.actor.items.get(sameItemId);
                        sameItem.delete();
                    }
 ...
 * @private
 */
    async onDropItemCreate(itemData) {
